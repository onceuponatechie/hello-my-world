...
import { ProjectStack, ProjectStackCard } from "@/components/project-stack";
  ...
  return (
    <section id="projects" aria-labelledby="projects-heading" className="relative scroll-mt-6 px-4 py-16 sm:px-8 sm:py-24">
      <div className="mx-auto max-w-5xl text-center">
        <h2 data-reveal="" id="projects-heading" className="font-serif text-[clamp(28px,3.6vw,44px)] italic leading-none tracking-tight">projects</h2>
      ...
      <ProjectStack>
        {projects.map((project, index) => {
          ...
          return (
            <ProjectStackCard key={project.name} index={index}>
              <article aria-labelledby={titleId} className={`group relative mx-auto max-w-6xl overflow-hidden rounded-[32px] p-4 ring-1 ring-black/5 shadow-[0_1px_2px_rgba(0,0,0,0.03),0_24px_44px_-32px_rgba(0,0,0,0.14)] md:rounded-[44px] md:p-5 ${project.tint}`}>
                <div className="grid h-full grid-cols-1 gap-4 md:grid-cols-2 md:gap-5">
                  <div data-reveal="right" className="relative order-1 aspect-[16/10] overflow-hidden rounded-[20px] md:order-2 md:aspect-auto md:min-h-[320px] md:rounded-[30px]">
                  ...
                  <div data-reveal="left" data-delay="100" className="order-2 flex flex-col justify-center gap-4 p-3 pb-0 pt-1 md:order-1 md:gap-5 md:p-7 lg:gap-5 lg:p-7">
                    ...
                    <h3 id={titleId} className="text-[clamp(1.7rem,3vw,2.6rem)] font-medium leading-[1.1] tracking-tight lg:text-[clamp(2.6rem,4vw,3.2rem)]">{project.name}</h3>
                    ...
                    <p className="max-w-[44ch] text-[14px] leading-[1.6] text-ink/65 md:text-[15px] lg:text-[17px]">{project.body}</p>
                    <div className="mt-1 flex flex-wrap items-center gap-4">
                      <Link href={`/projects/${project.slug}`} className="inline-flex items-center rounded-full bg-ink transition-colors hover:bg-ink/80 px-6 py-2.5 text-[12px] font-medium text-white lg:px-7 lg:py-4 lg:text-[14px]">View Case Study</Link>
      ...
      <div data-reveal="" className="mx-auto mt-10 flex max-w-6xl justify-center">
        <Link href="/projects" className="group inline-flex items-center gap-2 rounded-full border border-ink/15 px-6 py-3 text-[13px] font-medium transition-colors hover:bg-ink hover:text-white">Explore more projects <Icon name="arrow-up-right" className="h-4 w-4 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" /></Link>
      </div>
    </section>
  );
}
